...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { user_id, aula_id, quiz_score, missao_completa, status } = await req.json();
    ...
    let current_quiz_score = quiz_score !== undefined ? quiz_score : (prog?.quiz_score || 0);
    let current_missao_completa = missao_completa !== undefined ? missao_completa : (prog?.missao_completada || false);
    let current_status = status !== undefined ? status : (prog?.status || 'bloqueada');
    ...
    if (bothComplete && !is_already_completed) {
      ...
      if (aula.badge_nome) {
        ...
        if (badge) {
           await supabaseClient.from('user_badges').insert({ user_id, badge_id: badge.id });
           badgeConquistada = true;
        }
      }

      const { data: user } = await supabaseClient.from('users').select('xp').eq('id', user_id).single();
      if (user) {
        await supabaseClient.from('users').update({ xp: (user.xp || 0) + 100 }).eq('id', user_id);
      }
      ...
      if (allAulas) {
        ...
        if (currentIndex !== -1 && currentIndex < allAulas.length - 1) {
          ...
          if (!existingNextProg) {
            await supabaseClient.from('user_progress').insert({ user_id, aula_id: nextAulaId, status: 'desbloqueada' });
          } else {
             await supabaseClient.from('user_progress').update({ status: 'desbloqueada' }).eq('id', existingNextProg.id);
          }
        }
      }
    }
    ...
    if (prog) {
      await supabaseClient.from('user_progress').update(updatePayload).eq('id', prog.id);
    } else {
      await supabaseClient.from('user_progress').insert({ user_id, aula_id, ...updatePayload });
    }
